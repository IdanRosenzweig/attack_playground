...
`scripts/render_config.py`, and containerssh mounts *that* - see *the guest-only name
...
this document names no port values, on purpose: the two config files above are where
they are set, and a number repeated in prose is a number that goes stale. read the
current ones with `cat .env` and `cat attack_network_endpoints.conf`.

the guests' name for the gateway is handled the same way - `config.yaml`'s `extrahosts`
entry defines it and `scripts/render_config.py hostname` hands it to the scripts. see
*the guest-only name for the gateway* below.
...
itself as running with nothing listening on the ssh port. verified on an aarch64 ubuntu
...
is published on `127.0.0.1` only, on `AUTH_PORT` from `.env`. note that a docker published port is DNATed in the
...
`stats_server.py` answers, as json on `http://127.0.0.1:$STATS_PORT/stats` (`STATS_PORT`
in `.env`), how many users are connected right now, how many were connected at some point during the last
hour, and how many have ever connected:

```
$ curl -s http://127.0.0.1:$STATS_PORT/stats
...
webhook - from another machine, tunnel it
(`ssh -L $STATS_PORT:127.0.0.1:$STATS_PORT <host>`).
...
an attack endpoint that is itself a container with a published port (`-p <endpoint
port>:80`) is a different code path. docker DNATs a published port in the `nat` table
**before routing**, so a guest packet to `<gateway>:<that port>` arrives in `FORWARD` addressed to the endpoint
...
is for. `verify_guest.sh` starts a published endpoint on the second port the config
allows - whichever that is - and proves the path.
...
```
172.24.0.2:34400 > 172.24.0.1:<port>   [S] ttl 64     # what the guest dialled
...
published deliberately. `verify.sh` reports every configured range that overlaps it -
as shipped, one of them does.

### the guest-only name for the gateway

inside a guest, one name resolves to the gateway ip of `attack_playground_net` - the
address the attack endpoints are exposed on. it is an `/etc/hosts` entry, configured in
`config.yaml` under `docker.execution.host.extrahosts` (docker's
`HostConfig.ExtraHosts`) and written by docker into every guest container it creates:
...
that entry is where the name is **defined**, and the only place it is written down.
nothing repeats it: `scripts/render_config.py hostname` reads it back out of
`config.yaml`, and `start.sh`, `verify.sh` and `verify_guest.sh` ask for it there -
the same way they ask `scripts/endpoints.py` for the configured ports. renaming the
host is an edit to that one line, and `./verify.sh` will follow it. run

```
python3 scripts/render_config.py hostname
```

to see what the guests currently resolve.
...
a hosts entry rather than a dns record, for three reasons:
  ...
  * **it exists only inside a guest.** the host's own resolver, the `containerssh` and
    ...
    resolve it to the gateway - against the gateway ip rather than "does not resolve
    at all", since the name is a real domain and a host with a resolver may well have
    an answer for it.
...
the name is an **alias, not a permission**. `<name>:<port>` works exactly when tcp on
that port of the gateway is in the allowlist; the name on tcp/22 is dropped exactly
...
range like `1000-99999` is rejected by iptables, and building the chain the other way round
...
resolve the guest name to the gateway), that nothing was persisted to
...
icmp, the other guest over ipv4 and ipv6 link-local) is not, the guest name resolves
...
from `.env`, and the name it probes comes from `config.yaml` - none of them are values
of its own. its test listener is bound to the gateway ip and serves an empty
...
nc -vz <gateway-ip> <allowed-port>   # allowed endpoint, must succeed
curl -s <gateway-ip>:<allowed-port>/ # ... and must be the *host* listener, not a
...
getent hosts <guest-name>            # must print the gateway ip - guests only
nc -vz <guest-name> <allowed-port>   # allowed endpoint, reached by name
nc -vz <guest-name> 22               # must fail - the name is an alias, not a permission
nc -vz <other-guest-ip> <port>       # must fail
nc -6 -vz <other-guest-link-local>%eth0 <port>  # must fail
```

`<allowed-port>` is any port `attack_network_endpoints.conf` allows
(`python3 scripts/endpoints.py ports 2` prints the first two) and `<guest-name>` is what
`python3 scripts/render_config.py hostname` prints.
